...
const HomePage = () => {
  ...
  // Fetch books data only if user is authenticated
  useEffect(() => {
    const fetchBooksData = async () => {
      ...
      if (isAuthenticated) {
        ...
        // If not authenticated, clear any existing book data
        setFeaturedBooks([]);
        setRecentBooks([]);
        setTopRatedBooks([]);
        setLoading(false);
  ...
  // Common hero section for both authenticated and non-authenticated users
                ...
                {[1, 2, 3].map((index) => (
                  ...
                  >
                    {loading ? (
                      ...
                      isAuthenticated && featuredBooks[index - 1]?.coverImageUrl ? (
                        <img
  ...
  // Content for authenticated users - Recent Books and Top Rated Books sections
  ...
  // Content for non-authenticated users - Features and CTA sections
  ...
  return (
    <>
      {/* Hero Section - Common for both authenticated and non-authenticated users */}
      {renderHeroSection()}
      
      <Container maxWidth="lg" sx={{ mb: 8 }}>
        {/* Conditional rendering based on authentication status */}
        {isAuthenticated ? renderAuthenticatedContent() : renderNonAuthenticatedContent()}
      </Container>
    </>
  );
};
